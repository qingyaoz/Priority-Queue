...
void testHiddenData() {
    ...
    struct HiddenDataComp {
        bool operator()(HiddenData const& d1, HiddenData const& d2) const {
...
void testUpdatePriorities() {
    ...
    // NOTE: If add more data to the vector, don't push the pointers
    //       until AFTER the vector stops changing size!
    for (auto& datum : data) {
        eecsPQ.push(&datum);
    }
